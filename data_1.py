...
from pylab import *
def adjustData(img,mask, data_augmentation = True, padding = True,target_size = (128,128)):

    if data_augmentation == True:
        ED_img, ED_mask = create_ED_dataset(img, mask, img_nums = 4)    # elastic transformation (4 images)
        flip_img, flip_mask = create_flip_set(img, mask)                # flip (7 images)
        rotate_img, rotate_mask = create_rotation_set(img, mask)        # rotation&crop (6 images)
        crop_img, crop_mask = crop_image_set(img, mask)                 # crop (5 images)
...
def image_processing(img, mask, target_size = (128, 128),augmentation = True,padding = True ):
    ...
    for i in range (img.shape[0]):
        new_imgs,new_masks = adjustData(img[i],mask[i], data_augmentation = augmentation, padding = padding, target_size = target_size)
...
def create_ED_dataset(img, mask, img_nums = 4,mode = 'orginal'):
    count = 0
    shape = img.shape
    shape_size = shape[:2]
    random_state = np.random.RandomState(None)
    ...
    while(count < img_nums):
        ...
        # if save == True:
        #row_img_store = r'D:\PROJECT\a_1\ED_%d.tif' % count
        #label_img_store = r'D:\PROJECT\a_2\ED_%d.tif' % count
...
def create_flip_set(img, mask,mode = 'orginal'):
    count = 0
    ...
    for j in range(len(rotate)):
        ...
        # save and check
        # row_img_store = r'D:\PROJECT\a_1\flip_%d.tif' % count
        # label_img_store = r'D:\PROJECT\a_2\flip_%d.tif' % count
...
def create_rotation_set(img, mask,mode = 'original'):
    ...
    for j in range(len(rotate)):
        ...
        # row_img_store = r'D:\PROJECT\a_1\rotate_%d.tif' % j
        # label_img_store = r'D:\PROJECT\a_2\rotate_%d.tif' % j
        # row_roi.save(row_img_store)
        # label_roi.save(label_img_store)
...
def crop_image_set(img, mask, mode = 'original'):
    g_count = 0
    ...
    for j in range(len(start_x)):
        ...
        # row_img_store = r'D:\PROJECT\a_1\crop_%d.tif' % g_count
        # label_img_store = r'D:\PROJECT\a_2\crop_%d.tif' % g_count
        # row_roi.save(row_img_store)
        # label_roi.save(label_img_store)
